...
- (UILabel *)labelWithFontSize:(CGFloat)size
{
    ...
    //label.font = [UIFont systemFontOfSize:size weight:20];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _lastUpdateTimeLabel.text = @"上次刷新时间：无记录";
        
        // 3.状态标签
        [self addSubview:_statusLabel = [self labelWithFontSize:12]];
        _statusLabel.font = [UIFont systemFontOfSize:12 weight:20];
...
- (void)setFrame:(CGRect)frame
{
    ...
    // 2.时间标签
    CGFloat lastUpdateY = statusY + statusHeight + 2;
