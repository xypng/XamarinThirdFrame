...
+ (DKColorPicker)DKColorPickerWithRGB:(NSArray<NSNumber *> *)values {
    NSArray<DKThemeVersion *> *themes = [DKColorTable sharedColorTable].themes;
    NSMutableArray<UIColor *> *colors = [[NSMutableArray alloc] initWithCapacity:themes.count];
    NSParameterAssert(values.count == themes.count);
    for (NSNumber *number in values) {
        UIColor *color = [UIColor colorWithRed:((float)(([number integerValue] & 0xFF0000) >> 16))/255.0 green:((float)(([number integerValue] & 0xFF00) >> 8))/255.0 blue:((float)([number integerValue] & 0xFF))/255.0 alpha:1.0];
        [colors addObject:color];
    }
    return ^(DKThemeVersion *themeVersion) {
        NSUInteger index = [themes indexOfObject:themeVersion];
        if (index >= themes.count) {
            return colors[[themes indexOfObject:DKThemeVersionNormal]];
        }
        return colors[index];
    };
}

+ (DKColorPicker)DKColorPickerWithColors:(NSArray<UIColor *> *)colors {
    NSArray<DKThemeVersion *> *themes = [DKColorTable sharedColorTable].themes;
