...
- (void)reloadColorTableFromPList {
    // Clear previos color table
    self.table = [[NSMutableDictionary alloc] init];
    self.themes = [[NSArray alloc] init];

    // Load color table file
    //NSString *tempfile = @"pattern.plist";
    NSString *filepath = [[NSBundle mainBundle] pathForResource:self.file.stringByDeletingPathExtension ofType:self.file.pathExtension];
    NSDictionary *dic = [NSDictionary dictionaryWithContentsOfFile:filepath];
    NSMutableArray *tempTheme = [[NSMutableArray alloc] init];
    [dic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        //[self.table setObject:obj forKey:key];
        NSDictionary *themeDic = (NSDictionary *)obj;
        NSMutableDictionary *tempDic = [[NSMutableDictionary alloc] init];

        [themeDic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            NSString *colorStr = (NSString *)obj;
            NSArray<NSString *> *tempArray = [colorStr componentsSeparatedByString:@","];
            [tempArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF != ''"]];
            UIColor *color;
            ...
            {
                float red = [[tempArray[0] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] floatValue]/255;
                float green = [[tempArray[1] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] floatValue]/255;
                float blue = [[tempArray[2] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] floatValue]/255;
                //没有透明度
            ...
            else if (tempArray.count>=4)
            {
                ...
                float blue = [[tempArray[2] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] floatValue]/255;
                //有透明度
                float alpha = [[tempArray[3] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] floatValue];
                color = [UIColor colorWithRed:red green:green blue:blue alpha:alpha];
            }
            [tempDic setObject:color forKey:key];

            //key加到主题中
            if (![tempTheme containsObject:key]) {
                [tempTheme addObject:key];
            }
        }];
        [self.table setObject:tempDic forKey:key];
        self.themes = [NSArray arrayWithArray:tempTheme];
    }];
}
...
- (void)setFile:(NSString *)file {
    _file = file;
    if ([file.pathExtension isEqual:@"plist"]) {
        [self reloadColorTableFromPList];
    }
    else {
