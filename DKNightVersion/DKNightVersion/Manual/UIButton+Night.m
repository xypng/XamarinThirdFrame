...
- (void)dk_setBackgroundColor:(DKColorPicker)picker forState:(UIControlState)state {
    [self setBackgroundColor:picker(self.dk_manager.themeVersion) forState:state];
    NSString *key = [NSString stringWithFormat:@"%@", @(state)];
    NSMutableDictionary *dictionary = [self.pickers valueForKey:key];
    if (!dictionary) {
        dictionary = [[NSMutableDictionary alloc] init];
    }
    [dictionary setValue:[picker copy] forKey:NSStringFromSelector(@selector(setBackgroundColor:forState:))];
    [self.pickers setValue:dictionary forKey:key];
}
            ...
            [dictionary enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull selector, DKColorPicker  _Nonnull picker, BOOL * _Nonnull stop) {
                ...
                [UIView animateWithDuration:DKNightVersionAnimationDuration
                                 animations:^{
                                     ...
                                     } else if ([selector isEqualToString:NSStringFromSelector(@selector(setBackgroundColor:forState:))]) {
                                         UIColor *resultColor = picker(self.dk_manager.themeVersion);
                                         [self setBackgroundColor:resultColor forState:state];
                                     }
                                 }];
            }];
        } else {
...
- (void)setBackgroundColor:(UIColor *)color forState:(UIControlState)state {
    [self setBackgroundImage:[[self class] imageFromColor:color] forState:state];
}
